...
type RPCServer struct {
	...
	eventChan     map[pb.Controller_ChannelServer]chan string
}
...
func (s *RPCServer) Channel(stream pb.Controller_ChannelServer) error {
	...
	s.eventChan[stream] = make(chan string)
	...
	go func() {
		...
		for {
			...
			s.eventChan[stream] <- "recieve pong"
		}
	}()
	...
	for {
		...
		case event := <-s.eventChan[stream]:
			log.Println("recieve event chan", event)
