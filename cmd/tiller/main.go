...
type RPCServer struct {
	...
	pbtiller.UnimplementedStreamServiceServer
	...
	eventC        map[pbtiller.StreamService_StreamServer]chan string
}
...
func (s *RPCServer) Init() {
	...
	s.eventC = make(map[pbtiller.StreamService_StreamServer]chan string)
}
...
func (s *RPCServer) Start() {
	for {
		...
		pbtiller.RegisterStreamServiceServer(s.grpcServer, s)
...
func (s *RPCServer) Channel(stream pbtiller.StreamService_StreamServer) error {
	s.logger.Println("Client connected")
	s.eventC[stream] = make(chan string)
	...
	go func() {
		...
		for {
			...
			s.logger.Println("Received Pong message:", in.Command)
			s.eventC[stream] <- in.Command
		}
	}()
	...
	for {
		...
		case event := <-s.eventC[stream]:
			log.Println("recieve event: ", event)
			stream.Send(&pbtiller.MessageResponse{
				Reply: "run command",
			})
